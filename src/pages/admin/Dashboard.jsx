...
export default function AdminDashboard() {
  ...
  const [adminDepartments, setAdminDepartments] = useState(["Select Department", "Loading..."])
  ...
  //   if (role === "admin") {
  //     return ["Select Department", "Sales","Accounts","Factory","Maintenance", "Office"]
  //   }
  ...
  const fetchAdminDepartments = async () => {
    try {
      ...
      // Process the sheet data to get unique values from column A
      const rows = data.table.rows

      // Extract values from column A (index 0), skip header row
      const columnAValues = rows
        .slice(1) // Skip header row
        .map(row => row?.c?.[0]?.v) // Get column A values
        .filter(value => value && value.trim() !== '') // Filter out empty values

      // Get unique departments and sort them
      const uniqueDepartments = [...new Set(columnAValues)].sort()

      // Set the admin departments
      setAdminDepartments(["Select Department", ...uniqueDepartments])

    } catch (error) {
      console.error("Error fetching admin departments:", error)
      // Fallback to default departments if fetch fails
      setAdminDepartments(["Select Department", "Sales", "Accounts", "Factory", "Maintenance", "Office"])
    }
  }

  // UPDATED: Function to get accessible departments based on user role
  const getAccessibleDepartments = () => {
    const role = sessionStorage.getItem('role') || 'user'
    const currentUsername = sessionStorage.getItem('username') || ''

    // Admin can see departments dynamically from Whatsapp sheet column A
    if (role === "admin") {
  ...
  useEffect(() => {
    ...
    const role = sessionStorage.getItem('role') || 'User'
    // Format the role with first letter capitalized
    const formattedRole = role.charAt(0).toUpperCase() + role.slice(1).toLowerCase()
    setLoggedInUserRole(formattedRole)
  }, [])

  // UPDATED: useEffect to fetch admin departments when role is admin
  useEffect(() => {
    const role = sessionStorage.getItem('role') || 'user'

    // Only fetch admin departments if user is admin
    if (role === "admin") {
      fetchAdminDepartments()
    }

    // Also fetch user departments for regular users
    fetchUserDepartments()
  }, [])

  // UPDATED: useEffect to update master sheet options when admin departments or user department mapping changes
  useEffect(() => {
    const role = sessionStorage.getItem('role') || 'user'

    if (role === "admin" && adminDepartments.length > 2) {
      // For admin, use the dynamically fetched departments
      setMasterSheetOptions(adminDepartments)
      if (!selectedMasterOption && adminDepartments[1] !== "Loading...") {
        setSelectedMasterOption(adminDepartments[1]) // Set first department as default
      }
    } else if (Object.keys(userDepartmentMap).length > 0) {
      // For regular users, use the role-based departments
      const accessibleDepts = getAccessibleDepartments()
      setMasterSheetOptions(accessibleDepts)
    }
  }, [adminDepartments, userDepartmentMap])
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <h1 className="text-2xl font-bold tracking-tight text-purple-500"> {loggedInUserRole}  Dashboard</h1>
          <div className="flex items-center gap-2">
            ...
            >
              <option value="checklist">Recurring Task</option>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-lg border border-l-4 border-l-blue-500 shadow-md hover:shadow-lg transition-all bg-white">
            ...
            <div className="p-4">
              ...
              <p className="text-xs text-blue-600">
                {dashboardType === "delegation"
                  ? "Total tasks in delegation sheet"
                  : selectedMasterOption !== "Select Department"
                    ? `Total tasks in ${selectedMasterOption} (up to today)`
                    : "Select a department"}
              </p>
