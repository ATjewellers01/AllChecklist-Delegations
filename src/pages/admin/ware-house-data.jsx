...
//   PAGE_CONFIG: {
//     title: "Factory Tasks",
//     historyTitle: "Factory Task History",
...
//           { id: "col6", label: "Planned Date", type: "date" },
                        ...
                        <tr key={account._id} className={`${isSelected ? "bg-purple-50" : ""} hover:bg-gray-50`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap bg-yellow-50">
                            <div className="text-sm text-gray-900">
                              {(() => {
                                const delayDays = calculateDelayDays(account["col6"])
                                if (delayDays > 0) {
                                  return (
                                    <span className="flex items-center">
                                      <span className="text-red-600 font-semibold">{delayDays} days</span>
                                      <span className="ml-1 text-xs text-red-500">delayed</span>
                                    </span>
                                  )
                                } else if (account["col6"]) {
                                  const plannedDate = parseDateFromDDMMYYYY(account["col6"])
                                  const today = new Date()
                                  today.setHours(0, 0, 0, 0)
                                  if (plannedDate) {
                                    plannedDate.setHours(0, 0, 0, 0)
                                    if (plannedDate.getTime() === today.getTime()) {
                                      return <span className="text-blue-600 font-medium">Due Today</span>
                          ...
                          <td className="px-8 py-4 whitespace-nowrap bg-yellow-50">
                            <select
                              disabled={!isSelected}
                              value={additionalData[account._id] || ""}
                              onChange={(e) => {
                          ...
                          <td className="px-8 py-4 whitespace-nowrap bg-orange-50">
                            <input
                              type="text"
                              placeholder="Enter remarks"
                              disabled={!isSelected || !additionalData[account._id]}
                              value={remarksData[account._id] || ""}
                              onChange={(e) => setRemarksData((prev) => ({ ...prev, [account._id]: e.target.value }))}
                              className="border rounded-md px-2 py-1 w-full min-w-[200px] border-gray-300 disabled:bg-gray-100 disabled:cursor-not-allowed"
                            />
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap bg-green-50">
                            {account.image ? (
