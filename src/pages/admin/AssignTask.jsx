...
const CalendarComponent = ({ date, onChange, onClose }) => {
  ...
  const prevMonth = () => {
    ...
    // Only allow navigation if the previous month is not before the current month
    if (prevMonthDate.getMonth() >= today.getMonth() &&
    ...
    <div className="p-2 bg-white border border-gray-200 rounded-md shadow-md">
      <div className="flex justify-between items-center mb-2">
        <button
          ...
          className={`p-1 rounded-full ${isPrevMonthDisabled()
            ? "text-gray-400 cursor-not-allowed"
            : "hover:bg-gray-100"
            }`}
        >
